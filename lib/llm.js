...
/**
 * 尝试从已知框架配置文件中读取当前默认模型的 baseUrl / apiKey / model。
 * 依次尝试各框架，首个成功的结果即返回，失败时静默返回 null。
 *
 * 支持的框架配置（按优先级）：
 *   1. OpenClaw  — ~/.openclaw/openclaw.json
 *      格式：agents.defaults.model.primary = "provider/modelId"
 */
function readFrameworkModel() {
  // ── OpenClaw ──────────────────────────────────────────────
  try {
    ...
    if (primary) {
      ...
      if (slashIdx !== -1) {
        ...
        if (provider?.apiKey && provider?.baseUrl) {
          return { baseUrl: provider.baseUrl, apiKey: provider.apiKey, model: modelId, source: 'OpenClaw' };
        }
      }
    }
  } catch (_) {}
...
// 启动时读取一次，之后缓存
const _frameworkModel = readFrameworkModel();

/**
 * 调用 LLM chat completions。
 * 优先级：config.js MODEL_CONFIG → 框架配置文件（兜底）
...
async function callLLM(messages, opts = {}) {
  const baseUrl = opts.baseUrl || MODEL_CONFIG.base_url || _frameworkModel?.baseUrl;
  const model   = opts.model   || MODEL_CONFIG.model    || _frameworkModel?.model;
  const apiKey  = opts.apiKey  || MODEL_CONFIG.api_key  || _frameworkModel?.apiKey;
...
module.exports = { callLLM, withRetry, extractJson, readFrameworkModel };
