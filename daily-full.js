 ...
 *
 * 用法：
 *   node daily-full.js               # 默认：昨日 08:00 → 今日 08:00（8点定时任务）
 *   node daily-full.js today         # 今日模式：今日 08:00 → 当前时间
 *   node daily-full.js 2026-04-06    # 指定日期：该日 08:00 → 次日 08:00
 */
...
async function main() {
  ...
  console.error(`[daily-full] 报告日期: ${date}，window: ${window}`);
...
main()
  .catch(e => { console.error('[daily-full] 致命错误:', e.message || e); process.exit(1); })
  .finally(() => endPool());
